...
train_signalHsmms <- function(seqs) {
  ...
  signalHsmmNOCNODEG <- train_hsmm(seqs, aas, region_fun = find_nhc4)
  
  # degeneration
  signalHsmm <- train_hsmm(seqs, aaaggregation)
  #signalHsmmNOC <- train_hsmm(seqs, aaaggregation, region_fun = find_nhc4)
  
  #homology 50
  seq50f <- cdhit(seqs, thresh = 0.5, word_length = 2, only_signal = TRUE)
  signalHsmmNOHOM50 <- train_hsmm(seqs[seq50f], aaaggregation)
  #signalHsmmNOCNOHOM50 <- train_hsmm(seqs, aaaggregation, region_fun = find_nhc4)
  
  #homology 90
  seq90f <- cdhit(seqs, thresh = 0.9, word_length = 5, only_signal = TRUE)
  signalHsmmNOHOM90 <- train_hsmm(seqs[seq90f], aaaggregation)
  #signalHsmmNOCNOHOM90 <- train_hsmm(seqs[seq90f], aaaggregation, region_fun = find_nhc4)
